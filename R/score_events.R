...
score_events <-
  function(duration = 15, save = TRUE, file = "score.events.txt", vect = FALSE) {
    ...
    score.events <- as.data.frame(matrix(score.events,
      byrow = TRUE,
    ...
    if (vect) {
      ...
      if (event %in% levels(score.events[, 1])) {
        ...
        message(paste("Symbol", event, "was not recorded, returing a matrix. Function bin.event() will convert it into a vector."))
      }
    }
    return(score.events)
  }
